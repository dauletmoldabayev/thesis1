...
def compute_parameter(parameters, direction, extent):
    """
    Computes gamma_0 + theta * direction.
    """
    return (parameters[0] + extent*direction[0],
            parameters[1] + extent*direction[1])
...
class Solver(object):
    ...
    def solve(self, guess_wave, parameter_anchor, direction):
        ...
        guess = self.construct(guess_wave, variables = np.zeros(self.boundary.variables_num()), extent = 0)
